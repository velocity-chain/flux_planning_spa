    ...
    <v-row>
      <v-col cols="12">
        <ListPageSearch
          ...
          automation-id="supplier-list-search"
        />
      </v-col>
    </v-row>
        ...
        <v-card>
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="supplier-list-load-more"
            >
...
/**
 * Suppliers List Page - Showcase of flux_spa_utils simplicity
 * 
...
const {
  ...
  queryKey: ['suppliers'],
  queryFn: (params) => api.getSuppliers(params),
  getItemId: (item) => item._id,
  limit: 20,
})

function navigateToSupplier(_event: unknown, { item }: { item: Supplier }) {
  router.push(`/suppliers/${item._id}`)
}
