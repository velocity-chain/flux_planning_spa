<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Scenarios</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <ListPageSearch
          ...
          automation-id="scenario-list-search"
        />
      </v-col>
      <v-col cols="12" md="6" class="d-flex justify-end align-center">
        <v-btn 
          color="primary" 
          to="/scenarios/new"
          data-automation-id="scenario-list-new-button"
        >
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(scenarios ?? []) as unknown as Scenario[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToScenario"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="scenario-list-load-more"
            >
...
/**
 * Scenarios List Page - Showcase of flux_spa_utils ease of use
 * 
...
const {
  ...
  setSortBy,
  setOrder,
} = useInfiniteScroll<Scenario>({
  queryKey: ['scenarios'],
  queryFn: (params) => api.getScenarios(params),
  getItemId: (item) => item._id,
  limit: 20,
})

// Simple navigation handler
function navigateToScenario(_event: unknown, { item }: { item: Scenario }) {
  router.push(`/scenarios/${item._id}`)
}
