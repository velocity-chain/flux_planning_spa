<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Analyticss</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12">
        <ListPageSearch
          ...
          automation-id="analytics-list-search"
        />
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(analyticss ?? []) as unknown as Analytics[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToAnalytics"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="analytics-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load analyticss: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Analyticss List Page - Showcase of flux_spa_utils simplicity
 * 
...
const {
  ...
  setSortBy,
  setOrder,
} = useInfiniteScroll<Analytics>({
  queryKey: ['analyticss'],
  queryFn: (params) => api.getAnalyticss(params),
  getItemId: (item) => item._id,
  limit: 20,
})

function navigateToAnalytics(_event: unknown, { item }: { item: Analytics }) {
  router.push(`/analyticss/${item._id}`)
}
